...
const NewTaskButton = () => {
  ...
  const handleSubmitToDo: FormEventHandler<HTMLFormElement> = async (e) => {
    e.preventDefault();
    if (!title.trim() || !desc.trim() || !secretKey.trim()) {
      //alert("Title and Description are required.");
      // setShowAlert(true);
      if (!toast.isActive("crEmpty")) {
        toast.error("Title, Description and Secret key are required.", {
          containerId: "a",
          toastId: "crEmpty",
        });
      }
    ...
    try {
      const res = await fetch(`${baseUrl}/api/todos`, {
        ...
        body: JSON.stringify({ title, desc, prio, secretKey }),
        cache: "no-store",
      });
  ...
  return (
    <div className="grid grid-cols-8 gap-4">
      ...
      <Modal openModal={openModal} setOpenModal={handleModalClose}>
        ...
        >
          <h1 className="text-xl font-bold mb-5 max-w-4xl mx-auto">
            Create New ToDo!
          </h1>
          <input
            type="text"
